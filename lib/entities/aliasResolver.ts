...
export async function resolveEntity(
  ...
  // Fuzzy search
  type BestMatch = { row: AliasRow; score: number; matched: string };
  ...
  const match: BestMatch = best;
  return {
    canonical: match.row.canonical,
    type: match.row.type,
    confidence: Number(match.score.toFixed(3)),
    matched: term,
    aliases: ensureArrayAliases(match.row.aliases),
    metadata: (match.row.metadata ?? null) as Json | null,
    source: match.row.source ?? null,
  };
}
