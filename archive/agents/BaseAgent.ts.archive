...
import { getResolvedPreferences, type ResolvedPrefs } from '../../../lib/preferences/store';
...
`;
  ...
  protected buildLearnedPreferencesSection(prefs: ResolvedPrefs): string {
...
This is a new user. Pay attention to their behavior to learn preferences:
- Which sections they engage with most
- Preferred research depth (quick vs deep)
- Output style preferences (concise vs detailed)
- Focus areas they consistently ask about

After this interaction, their preferences will begin to form.
...
The system has learned these preferences from ${prefs.coverage?.confidence || 0} previous interactions:
...
- ${confidence > 0.8 ? 'ALWAYS use this depth unless explicitly overridden' : 
     confidence > 0.5 ? 'Prefer this depth but offer alternatives if ambiguous' :
     'Low confidence - ask for clarification on first request'}

`;
    }

    // Focus areas
    if (prefs.focus?.areas && prefs.focus.areas.length > 0) {
      section += `### Primary Focus Areas (user consistently engages with):
${prefs.focus.areas.map(area => `- **${area}**: Always include detailed coverage`).join('\n')}

`;
    }

    if (prefs.focus?.always_include && prefs.focus.always_include.length > 0) {
      section += `### ALWAYS Include These Sections:
${prefs.focus.always_include.map(s => `- ${s}`).join('\n')}

`;
    }

    if (prefs.focus?.skip_unless_relevant && prefs.focus.skip_unless_relevant.length > 0) {
      section += `### Skip Unless Highly Relevant (user rarely engages):
${prefs.focus.skip_unless_relevant.map(s => `- ${s}`).join('\n')}
...
`;
      if (prefs.summary.brevity === 'short') {
        section += `- Keep responses concise and scannable
- Use bullet points over paragraphs
- Prioritize key insights only
`;
      } else if (prefs.summary.brevity === 'long') {
        section += `- Provide comprehensive detail
- Include context and background
- Explain reasoning and methodology
`;
      } else {
        section += `- Balance detail with scannability
...
`;
    ...
    // Tone preference
    if (prefs.tone) {
      section += `### Communication Tone: ${prefs.tone}
`;
      if (prefs.tone === 'formal') {
        section += `- Professional, business-focused language
- Avoid casual expressions
`;
      } else if (prefs.tone === 'casual') {
        section += `- Conversational, approachable tone
- Use natural language
`;
      } else {
        section += `- Professional but approachable
...
`;
    ...
    section += `**IMPORTANT**: These preferences were learned from actual user behavior. Respect them unless the user explicitly requests something different.
