...
export function AccountListWidget({ onAccountClick, onAddAccount, onResearchAccount, onViewSetup, showFooter = true }: AccountListWidgetProps) {
  ...
  const filterOptions = useMemo(() => ([
    ...
    { value: 'hot' as const, label: 'Hot', count: stats?.hot ?? 0 },
    { value: 'warm' as const, label: 'Warm', count: stats?.warm ?? 0 },
    { value: 'stale' as const, label: 'Stale', count: stats?.stale ?? 0 },
  ]), [stats]);
    ...
    <div className="flex flex-col h-full" data-testid="account-list-widget">
      <div className="px-4 py-3 border-b border-gray-200">
        ...
        <div
          ...
          aria-describedby={(stats?.total ?? 0) === 0 ? 'account-priority-helper' : undefined}
        >
        ...
        <div className="px-4 py-2 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center justify-between text-xs text-gray-600">
            ...
            <span className="flex items-center gap-1">
              <AlertCircle className="w-3 h-3" />
              {stats?.with_signals ?? 0} with signals
            </span>
          </div>
        </div>
      )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-6 py-10">
      <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          >
            {[
              { value: 'all' as const, label: `All (${stats?.total ?? 0})` },
              { value: 'hot' as const, label: `Hot (${stats?.hot ?? 0})` },
              { value: 'warm' as const, label: `Warm (${stats?.warm ?? 0})` },
              { value: 'stale' as const, label: `Stale (${stats?.stale ?? 0})` },
            ].map(option => (
              <button
                key={option.value}
                type="button"
